...
namespace Preprocessing
{
    class ReadyFeaturesParser : Parser
    {
        ...
        // Constructor
        public ReadyFeaturesParser(ConfigurationManager configManager, Logger logger, string maxIDFullFileName)
...
//                        posFullFileName = GetPOSFullFileName(mrfFullFileName);
                        ...
                        for (int i = 0; (mrfTags != null) && (i < mrfTags.Length); i++)
                        {
                            ...
                            // Parse mrf tag
                            TagParse(mrfTags[i], out mrfWordName, out mrfIDs, false);

                            // Parse POS tag
                            if (posTags != null)
                            {
                                TagParse(posTags[i], out posWordName, out posIDs, false);
                            ...
                            if (mrfIDs != null)
                            {
                                MrfFillItemIDs(mrfIDs, ref tagWord);
                            }// end if
                            else
                            {
                                // Add nothing
                ...
                case "WordOnly":
                    ...
                    if (posTags == null)
                    {
                        logger.LogError("Configuration is WordOnly for ReadyFeatures. POS file or POS file is empty. Nothing will be added.", ErrorCode.FILE_IS_EMPTY);
                        return fileWordsList;
                    }
                    ...
                    logger.LogTrace("Words of file: " + mrfFullFileName + " are successufuly parsed for WordOnly target, no mrf or POS fields are parsed");
        ...
        protected override void TagParse(String tag, out String itemName, out int[] IDs, bool stanfordMapStanfordToRDI)
        {
            ...
            if (tag != null)
            {
                ...
                //String wordExpression = @"\{+\(?(?<itemName>.+?)\)?;"; //@"[^{].+?;"; // some characters followed by ;
                // \{+\s*\{*\(? matches: {
                String wordExpression = @"\{+\s*\{*\(?(?<itemName>.+?)\)?;"; //@"[^{].+?;"; // some characters followed by ;
                ...
                try
                {
                    itemName = Regex.Match(tag, wordExpression).Result("${itemName}");
                }
                catch
                {
                    itemName = null;
        ...
        private void MrfFillItemIDs(int[] IDs, ref Item word)
        {
            try
            {
            ...
            {
                logger.LogError("Arabized word: " + word.itemName, ErrorCode.ARABIZED_WORD);
            }// end try-catch
        } // end MrfFillItemIDs()
