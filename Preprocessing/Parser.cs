...
namespace Preprocessing
{
    abstract class Parser
    {
        ...
        public Parser(ConfigurationManager configManager, Logger logger)
        {
            ...
            //wordsLogFile = new StreamWriter(File.Open(@"D:\Work\Research\PhD\\Implementation\Diactrization\Preprocessing\items.txt", FileMode.OpenOrCreate, FileAccess.Write), Encoding.GetEncoding(1256));
                    ...
                    case "ClassifySyntDiac":
                        ...
                        if (classifiedWords != null)
                        {
                            ...
                            foreach (Feature classifiedWord in classifiedWords)
                            {
                                ...
                                {
                                    misClassifiedWords ++;
                                    fileMisClassifiedWords++;
                                }
                            }

                            logger.LogTrace("File misclassified items " + fileMisClassifiedWords + " out of " + classifiedWords.Length);
                            float accuracy = (((float)(classifiedWords.Length - fileMisClassifiedWords) / classifiedWords.Length) * 100);
                            logger.LogTrace("File Accuracy " + accuracy.ToString() + @"%");

                            logger.LogTrace("Overall Misclassified items " + misClassifiedWords + " out of " + totalNumWords);
                            accuracy = (((float)(totalNumWords - misClassifiedWords) / totalNumWords) * 100);
                            logger.LogTrace("Overall Accuracy " + accuracy.ToString() + @"%");
