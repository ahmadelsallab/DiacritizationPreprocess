...
namespace Preprocessing
{
    abstract class FeaturesFormatter
    {
        ...
        private String GetLastCharFeatures(String wordName)
        {
            if (wordName != null)
            {
        ...
        {           
            if ((wordName != null) && targetHashTable[(int)wordName.ToCharArray().Last()] != null)
            {
                //if
        ...
        protected String EmptyLastCharFeatureString()
        {
            ...
            for (int position = (int)configManager.lastCharFeaturesDepth * 43; position > 0; position--)
            {
                result = result + "0,";
            }// end for
            return result;
